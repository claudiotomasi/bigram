...
def adjacents_bigrams():
    # bi = {}
    # adj = adjacents()
    # for k1,v1 in adj.iteritems():
    #     for k2,v2 in adj.iteritems():
    #         bi[(k1, k2)] = list(it.product(v1,v2))
    # afile = open('adjBigrams', 'wb')
    # pickle.dump(bi, afile)
    # afile.close()
    afile = open('adjBigrams', 'rb')
    bi= pickle.load(afile)
    afile.close()
    return bi
